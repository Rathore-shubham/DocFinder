  ...
  return dashData && (
    <div className='m-5'>
      ...
      <div className='flex flex-wrap gap-3'>
        <div className='flex items-center gap-2 bg-zinc-900 text-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
        ...
        <div className='flex items-center gap-2 bg-zinc-900 p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
      ...
      <div className='bg-zinc-900'>
        ...
        <div className='pt-4 border border-t-0'>
          {dashData.latestAppointments.slice(0, 5).map((item, index) => (
            <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-900' key={index}>
              <img className='rounded-full w-10' src={item.docData.image} alt="" />
              <div className='flex-1 text-sm'>
                <p className='text-white font-medium'>{item.docData.name}</p>
                <p className='text-white '>Booking on {slotDateFormat(item.date)}</p>
              </div>
